...
local antiAfkEnabled = false  -- ตั้งค่า Anti-AFK
...
local function saveConfig()
    ...
    local data = {
        ...
        antiAfk = antiAfkEnabled  -- เก็บ anti-afk setting
    }  
...
local function loadConfig()
    ...
    antiAfkEnabled = data.antiAfk or false  -- โหลด anti-afk
...
task.spawn(function()
    while true do
        task.wait(30)  -- ทุก 30 วินาที
        if antiAfkEnabled then  -- เฉพาะเมื่อ Anti-AFK เปิด (ไม่ต้องดู Auto Farm)
            local char = player.Character
            if char then
                ...
                if hrp and humanoid then
                    ...
                    local randomDir = Vector3.new(math.random(-5, 5), 0, math.random(-5, 5))
...
-- ⏸️ Anti-AFK Toggle
local antiAfkToggle = settingsTab:Toggle('Anti-AFK (ทุก 30 วิ)', antiAfkEnabled, function(state)
    antiAfkEnabled = state
    saveConfig()
    if state then
        print("⏸️ Anti-AFK เปิดแล้ว - สุ่มเดิน + jump ทุก 30 วินาที")
    else
        print("⏸️ Anti-AFK ปิดแล้ว")
    end
end)
...
task.delay(0.5, function()
    ...
    if antiAfkToggle and antiAfkToggle.Set then antiAfkToggle:Set(antiAfkEnabled) end
